...
- **App Service worker count** — startup command in `deploy.yml` runs `uvicorn --workers 2` (was 4). On the B1 plan (1.75 GB) 4 workers over-subscribed memory and any heavy LLM run (verdict / buddies / threat model) OOM-killed a worker. 2 workers roughly halves peak memory pressure. If you bump the plan later, raise this in lock-step
