...
**Settings** (some admin-only): AI Settings · Email Settings · Sync · Access Logs (admin) · Administration · Help
...
- **Access Logs** (`/access-logs`, **global-admin only**) — audit trail of who accesses the portal. One `access_logs` row per authenticated API request (user email/name, method, path, status, IP, user-agent, timestamp), written best-effort by the HTTP middleware in `main.py` (decodes the bearer token off the response path via `run_in_threadpool`, so it never adds latency; skips OPTIONS/health/docs and the access-logs fetch itself). Endpoint `GET /admin/access-logs/` (`require_role(ADMIN)`) with user/method/path/since filters + pagination. 90-day retention pruned at startup (`_prune_access_logs`).
