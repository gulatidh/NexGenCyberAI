...
const TYPE_ICON: Record<string, React.ReactNode> = {
  ...
  info: <Info sx={{ fontSize: 16, color: "#A100FF" }} />,
};

const TYPE_COLOR: Record<string, string> = {
  success: "#00e676", error: "#f44336", warning: "#ff9800", info: "#A100FF",
};
  ...
  return (
    <>
      <Tooltip title="Notifications & Activity Log">
        <IconButton onClick={handleOpen} size="small" sx={{ mr: 1 }}>
          <Badge badgeContent={count} color="error" max={99}>
            {count > 0
              ? <Notifications sx={{ color: "#A100FF", fontSize: 22 }} />
      ...
      <Popover
        ...
        slotProps={{ paper: { sx: { bgcolor: "#1A1A1A", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 2, width: 400, maxHeight: 520 } } }}
      >
            ...
            {notifications.map((n, i) => (
              <React.Fragment key={n.id}>
                <ListItem sx={{ py: 1, px: 1.5, alignItems: "flex-start", gap: 1,
                  bgcolor: n.read ? "transparent" : "rgba(161,0,255,0.03)" }}>
